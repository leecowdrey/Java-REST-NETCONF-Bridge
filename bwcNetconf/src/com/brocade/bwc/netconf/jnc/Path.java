        ...
        try {
            ...
            while (sz > 0) {
                ...
                if (tok1.type == SLASH) {
                    ...
                    steps.add(step);
                    tokens.remove(0);
                } /* AXIS::PREFIX:TAG */ else if (tok1.type == ATOM && tok2.type == COLONCOLON
                ...
                } /* AXIS::TAG */ else if (tok1.type == ATOM && tok2.type == COLONCOLON
                        && tok3.type == ATOM) {
                    step = new LocationStep(parseAxis(tok1.value), tok3.value);
                    steps.add(step);
                    tokens.removeRange(0, 3);
                    parsePredicates(tokens, step);
                } /* PREFIX:TAG */ else if (tok1.type == ATOM && tok2.type == COLON
                        && tok3.type == ATOM) {
                    ...
                    tokens.removeRange(0, 3);
                    parsePredicates(tokens, step);
                } /* TAG */ else if (tok1.type == ATOM) {
                    step = new LocationStep(AXIS_CHILD, tok1.value);
                    steps.add(step);
                    tokens.remove(0);
                    parsePredicates(tokens, step);
                } /* '//' (SLASHSLASH) */ else if (tok1.type == SLASHSLASH) {
        ...
        while (i < to) {
            ...
            if (tok1.type == ATOM && (tok2 != null ? tok2.type : 0) == COMPARE && tok3 != null) {
                ...
                return new Expr(tok2.op, new Expr(CHILD_VALUE, tok1.value),
                        rexpr);
            } /* ATTR = */ else if (tok1.type == ATTR && (tok2 != null ? tok2.type : 0) == COMPARE
                    && tok3 != null) {
                final Object rexpr = parsePredicate_rvalue(tokens, i + 2, to);
