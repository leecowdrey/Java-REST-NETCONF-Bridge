 ...
 * {@link com.brocade.bwc.netconf.jnc.Element Element}. It is an extension of
 * the Element class to make the configuration sub-tree data model aware.
 * Classes generated from the JNC pyang plugin are either YangElements or
 * derived data types. Thus the YangElement which is an abstract class is never
 * used directly.
 * <p>
 ...
 * <ul>
 * <li>{@link #checkSync checkSync} - checks if two configurations are equal, or
 * if a sync is needed. This is a typical operation a manager would like to do
 * when one configuration is stored on the device and the other is stored in a
 * database.
...
public abstract class YangElement extends Element {
    ...
    /**
     * Returns a list of subtrees for syncing a subtree A with all the necessary
     * operations to make it look like the target tree B. This variant uses the
