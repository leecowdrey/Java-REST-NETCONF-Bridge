 ...
 * href="ftp://ftp.rfc-editor.org/in-notes/rfc4741.txt"&gt;RFC 4741&lt;/a&gt;]
 * can be performed, e.g. <code>get-config</code>, <code>edit-config</code> and
 * <code>commit</code>.
 * <p>
 ...
 * @see Element
 *
 * LC: lcowdrey@cisco.com - added GET-SCHEMA
 *
 *
 */
public class NetconfSession {
    ...
    // LC added below
    public static final String GET_SCHEMA_GT = "get-schema>";
    public static final String GET_SCHEMA = "get-schema";
    public static final String IDENTIFIER_GT = "identifier>";
    public static final String IDENTIFIER = "identifier";
    public static final String FORMAT_GT = "format>";
    public static final String FORMAT = "format";
    public static final String VERSION_GT = "version>";
    public static final String VERSION = "version";
    public static final String LOCATION_GT = "location>";
     ...
     *
     * If we are using the confm package to create and manipulate objects we
     * must instantiate the parser as an com.tailf.confm.XMLParser() If we fail
     * to do that, we get the XMLParser class from the inm package which always
     * only returns Element objects as opposed to the XMLParser from the confm
     * package which can return objects matching the confm generated classes. If
     * we use the confm Device class to create our netconf sessions, this is all
     * done automatically, whereas if we create our netconf sessions ourselves,
     * _and_ also want to retrieve real confm objects we must:
     *
     * <pre>
     ...
     * LC added
     *
     * Retrieves YANG module defined via capabilities and advertised as module.
     */
    public NodeSet getSchemaConformance() throws JNCException, IOException {
        trace("get-schema-conformance: ");
        final int mid = encode_getSchemaConformance(out);
        out.flush();
        return recv_rpc_reply_data(mid);
    }
     ...
     * Retrieves YANG module defined via capabilities and advertised as module.
     */
    public String getSchema(String identifier, String version, String format) throws JNCException, IOException {
        String definition = null;
        trace("get-schema: ");
        encode_getSchema(out, identifier, version, format);
        out.flush();
        ...
        if (data == null) {
            trace(identifier + ": \n" + "no module source available");
            throw new JNCException(JNCException.MODULE_SCHEMA_NOT_FOUND,
                    "get-schema contains no module definition for " + identifier);
        } else {
            definition = data.getValue().toString();
            trace(identifier + ": \n" + data.toString());
        }
        return definition;
    }
     ...
     * LC added
     *
     * Issues adhoc "action" RPC XML within a RPC block, without waiting for
     * response
     */
    public void actionRpc(String rpcConfig) throws JNCException,
            IOException {
        trace("actionRpc: " + rpcConfig);

        final String prefix = Element.defaultPrefixes.nsToPrefix(Element.NETCONF_NAMESPACE);
        nc = mk_prefix_colon(prefix);
        final String xmlnsAttr = mk_xmlns_attr(prefix, Element.NETCONF_NAMESPACE);
        final String actionXmlnsAttr = mk_xmlns_attr("", Capabilities.NS_ACTIONS);
        ...
        out.print("<" + Element.ACTION + " " + actionXmlnsAttr + ">");
        if (rpcConfig != null) {
            out.print(rpcConfig);
        }
        out.println("</" + Element.ACTION + ">");
        out.print("</" + nc + "rpc>");
    ...
    /**
     * Edits the configuration. The &lt;edit-config&gt; operation loads all or
     * part of a specified configuration to the specified target configuration.
     *
     * @param datastore The target datastore. One of {@link #RUNNING},
     *            {@link #CANDIDATE}, {@link #STARTUP}
     ...
     * <ul>
     * <li> {@link #MERGE}: The configuration data is merged with the
     * configuration at the corresponding level in the target datastore.
     * <li> {@link #REPLACE}: The configuration data completely replaces the
     * configuration in the target datastore. This is useful for loading
     * previously saved configuration data.
     * <li> {@link #CREATE}: The configuration data completely creates the
     * configuration in the target datastore. LC added
     * <li> {@link #DELETE}: The configuration data completely deletes the
     * configuration in the target datastore. If configuration does not exist
     * then error is raised. LC added
     * <li> {@link #REMOVE}: The configuration data completely deletes the
     * configuration in the target datastore. If configuration does not exist
     * then no error is raised. Requires :base:1.1 capability. LC added
     * <li> {@link #NONE}: The target datastore is unaffected by the
     * configuration parameter, unless and until the incoming configuration data
     * uses the "operation" attribute to request a different operation. If the
     ...
     * <ul>
     * <li> {@link #TEST_THEN_SET}: Perform a validation test before attempting
     * to set. If validation errors occur, do not perform the "edit-config"
     * operation. This is the default test-option.
     * <li> {@link #SET}: Perform a set without a validation test first.
     * <li> {@link #TEST_ONLY}: Only test. (Option is not supported in the
     * standard.)
     * </ul>
     *
     ...
     * <ul>
     * <li> {@link #STOP_ON_ERROR}: Abort the edit-config operation on first
     * error. This is the default error-option.
     * <li> {@link #CONTINUE_ON_ERROR}: Continue to process configuration data
     * on error; error is recorded, and negative response is generated if any
     * errors occur.
     * <li> {@link #ROLLBACK_ON_ERROR}: If an error condition occurs such that
     * an error severity "rpc-error" element is generated, the server will stop
     ...
     * the candidate configuration to the current running configuration.
     */
     ...
     * <code>null</code> if the subscription is not a replay subscription
     * @param stopTime a dateTime string specifying the stop of replay, or
     * <code>null</code> if the subscription is not a replay subscription or
     * infinite subscription is desired
     ...
     * LC added
     *
     * Encode the &lt;getSchemaConformance&gt;. Example:
     *
     * <pre>
     * &lt;rpc message-id="101"
     *        xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"&gt;
     ...
     * </pre>
     */
    int encode_getSchemaConformance(Transport out)
            throws JNCException {
        final int mid = encode_rpc_begin(out, withDefaultsAttr);
        out.println("<" + nc + "get-schema-conformance " + "/>");
        encode_rpc_end(out);
        return mid;
    }
     ...
     * LC: added
     *
     * Encode the &lt;getSchema&gt;. Example:
     *
     * <pre>
     * &lt;rpc message-id="101"
     *        xmlns="urn:ietf:params:xml:ns:netconf:base:1.0"&gt;
     *     &lt;get-schema xmlns="urn:ietf:params:xml:ns:yang:ietf-netconf-monitoring"&gt;
     *     &lt;identifier&gt;module-identiifer&lt;/identifier&gt;
     *     &lt;format&gt;yang&lt;/format&gt;
     *     &lt;/get-schema&gt;
     * &lt;/rpc&gt;
     * </pre>
     */
    int encode_getSchema(Transport out, String identifier, String version, String format)
            throws JNCException {
        /*
         RFC, request:
 ...
 /* http://www.tail-f.com/publish-your-data-models/
        ...
        out.print("<" + GET_SCHEMA + " xmlns=\"" + Element.NETCONF_YANG_NAMESPACE + "\">");
        out.print("<" + IDENTIFIER_GT + identifier + "</" + IDENTIFIER_GT);

        // version is actually revision
        if (version != null) {
            if (!version.isEmpty()) {
                out.print("<" + VERSION_GT + version + "</" + VERSION_GT);
            }
        }

        // RFC acceptable formats are: xsd, yang, yin, rng, rnc
        // if not supplied force yang
        if (format.equalsIgnoreCase("xsd")
                || format.equalsIgnoreCase("yang")
                || format.equalsIgnoreCase("yin")
                || format.equalsIgnoreCase("rng")
                || format.equalsIgnoreCase("rnc")) {
            out.print("<" + FORMAT_GT + format + "</" + FORMAT_GT);
        } else {
            out.print("<" + FORMAT_GT + "yang" + "</" + FORMAT_GT);
        }

        out.println("</" + GET_SCHEMA_GT);
        encode_rpc_end(out);
        return mid;
    }
    ...
    void encode_defaultOperation(Transport out) throws JNCException {
        switch (defaultOperation) {
            ...
            // LC added
            case CREATE:
                out.println("<" + nc + "default-operation>create</" + nc
                        + "default-operation>");
                return;
            // LC added
            case DELETE:
                out.println("<" + nc + "default-operation>delete</" + nc
                        + "default-operation>");
                return;
            // LC added
            case REMOVE:
                out.println("<" + nc + "default-operation>remove</" + nc
                        + "default-operation>");
                return;
            case NONE:
     ...
     *
     * @param prefix
     * @return "unknown:" if prefix is null, an empty string if prefix is empty,
     * otherwise prefix appended with a colon.
     */
    ...
    /**
     * Checks that message id attribute of t is mid. If mid is null, no check is
     * performed.
     *
