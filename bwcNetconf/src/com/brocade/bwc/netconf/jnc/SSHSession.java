 ...
 * to use SSH for its transport. This class uses the Ganymed SSH implementation. (<a
...
public class SSHSession implements Transport {
     ...
     * affects all read operations. If a timeout is reached, an INMException is
     ...
     * function can typically be used to poll a socket and see there is data to
     * be read. The function will also return true if the server side has closed
     * its end of the ssh socket. To explictly just check for that, use the
     * serverSideClosed() method.
     */
    @Override
